...
melinka_database = database.loc[database['NGAsubEQID'] == eqid]
melinka_database = melinka_database.sort_values(by='ClstD_km')

R_gmm = np.logspace(np.log10(30),np.log10(1000),100)
Rref_gmm = functions_py.get_R_Rref_interface(melinka_database['Earthquake_Magnitude'].unique(),R_gmm)[1]
input_df = pd.DataFrame({"mag": len(R_gmm) * [melinka_database['Earthquake_Magnitude'].unique()[0]], "rrup" : R_gmm, 
                            "vs30": len(R_gmm)*[760], "region": len(R_gmm)*['SA'], "saturation_region": len(R_gmm)*['SA_S'],"backarc":len(R_gmm)*[False]})
n = input_df.shape[0]
ctx = np.recarray(n,
...
#gmt grdgradient '/home/jtommy/Escritorio/Respaldo/base_de_datos/Topobatimetrias/melinka.nc' -G'/home/jtommy/Escritorio/Respaldo/base_de_datos/Topobatimetrias/melinka.int' -A225 -Nt

periods_plot = [0]##[-1,0,0.5,0.75,3,10]
R_gmm_v2 = functions_py.get_R_Rref_interface(melinka_database['Earthquake_Magnitude'].unique(),R_gmm)[0]
...
for T in periods_plot:
    ...
    melinka_df = functions_py.get_event_period_df(database,Fs,eqid,T,coeff_Rp,coeff_Rrup,p_value=p_opt_T,Rp_sheets=Rp_sheets,Rp_sheets_lock=Rp_lock_sheets,region = 'Global',Rref = Rref_gmm) 
    melinka_df = melinka_df.dropna(subset='log_obs_rock')
    obs_T = melinka_df['log_obs_rock']
    mag = np.array(len(R_gmm)*[melinka_df['Earthquake_Magnitude'].unique()[0]])
    R = melinka_df['R']
    Rrup = melinka_df['Rrup']
    Rp = melinka_df['Rp']
    Rp_lock = melinka_df['Rp_lock']
    ...
    pred_Rp = functions_py.DR_pred_GMM_Rp(R_gmm,mag,coeff_Rp_T,Rref = Rref_gmm)
    pred_Rp_regional = functions_py.DR_pred_GMM_Rp(R_gmm,mag,coeff_Rp_regional_T,Rref = Rref_gmm, region = 'SA')
    #### GMMs externas ###
    ...
    ln_mean_parker = ln_mean_parker.reshape((len(R_gmm),))
    ln_mean_montalva = ln_mean_montalva.reshape((len(R_gmm),))
    fig1 = pygmt.Figure()
    ...
    with fig1.subplot(nrows = 1, ncols = 1, figsize=("4c","12c")):
        with fig1.set_panel(panel = [0,0]):
            fig1.basemap(region=[-76, -72,-45,-42], projection=projection_geo, frame=['WSne','xa1f.5','ya1f.5'])        
            pygmt.makecpt(cmap='/home/jtommy/Escritorio/graficos_GMT/paletas/grayscale02.cpt',series = [-30000,6000,10])
            fig1.grdimage(grid ='/home/jtommy/Escritorio/Respaldo/base_de_datos/Topobatimetrias/melinka.nc',projection = projection_geo,
                        shading ='/home/jtommy/Escritorio/Respaldo/base_de_datos/Topobatimetrias/melinka.int', cmap=True )
            ...
            fig1.colorbar(frame="xaf+lLocking degree",position="g-75.8/-45.5+w7c/0.5c+h",projection=projection_geo,region=[-76, -72,-45,-42])
            ...
            fig1.plot(x=melinka_df['Station_Longitude_deg'], y=melinka_df['Station_Latitude_deg'], fill='green',style='t0.2c',projection = projection_geo,pen="0.1p,black")
            fig1.text(text="a) MM11", position='TL', font="8p,Helvetica,black", projection=projection_geo,fill="white",pen = "0.25p,black,solid",offset='0.1/-0.1')
            for i in modelos_cosismicos['event_tag']:
                ...
                fig1.grdcontour(grid = model_grid,limit = [0,slip_th30],projection = projection_geo,annotation = "n",levels=slip_th30,pen='1p,blue')#+colores[cont])
                ...
                cont = cont+1
    fig1.shift_origin(xshift="w+6c",yshift="-1.4c")
    with fig1.subplot(nrows = 3, ncols = 1, figsize=("4c","10c"),sharex=True):
        pygmt.config(FONT="6p,Helvetica,black")
        with fig1.set_panel(panel = [0,0]):
            fig1.basemap(region=[30, 1000,min(min(obs_T),min(pred_Rp),min(ln_mean_parker),min(ln_mean_montalva)) -1.2,max(max(obs_T),max(pred_Rp),max(ln_mean_parker),max(ln_mean_montalva))+1], projection=projection_log, frame=['Wsne','xa2fg3','ya2fg3'])
            fig1.plot(x = Rp_lock,y = obs_T,style='c0.1c',fill='gray',pen='0.1p,black',intensity = 0.3)
            #fig1.plot(x = R_gmm,y = pred_Rp,pen='1p,red',label='This model (Global)') 
            fig1.plot(x = R_gmm,y = pred_Rp_regional,pen='1p,red',label='This model')                
            ...
            fig1.legend(position="JBL+jBL+o0.1c+w1.9c",box = "+gwhite+p1p")      
        ...
        with fig1.set_panel(panel = [2,0]):
            ...
            #fig1.plot(x = R_gmm,y = pred_Rrup,pen='1p,red',label = 'This model')
            fig1.plot(x = R_gmm,y = pred_Rrup_regional,pen='1p,red',label = 'This model')
            fig1.plot(x = R_gmm,y = ln_mean_parker,pen='1p,blue', label='Parker et al. (2021)')
            fig1.plot(x = R_gmm,y = ln_mean_montalva,pen='1p,black', label='Montalva et al. (2017)')
...
fig1.savefig('/home/jtommy/Escritorio/Respaldo/Paper1_v2/Predicciones/Melinka/pred_melinka.png',dpi=300,transparent=True)
fig1.savefig('/home/jtommy/Escritorio/Respaldo/Paper1_v2/Predicciones/Melinka/pred_melinka.pdf')