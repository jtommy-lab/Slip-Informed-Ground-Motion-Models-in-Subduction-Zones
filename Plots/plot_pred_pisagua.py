...
pisagua_database = database.loc[database['NGAsubEQID'] == eqid]
pisagua_database = pisagua_database.sort_values(by='ClstD_km')

R_gmm = np.logspace(np.log10(30),3,100)
Rref_gmm = functions_py.get_R_Rref_interface(pisagua_database['Earthquake_Magnitude'].unique(),R_gmm)[1]
input_df = pd.DataFrame({"mag": len(R_gmm) * [pisagua_database['Earthquake_Magnitude'].unique()[0]], "rrup" : R_gmm, 
...
R_gmm_v2 = functions_py.get_R_Rref_interface(pisagua_database['Earthquake_Magnitude'].unique(),R_gmm)[0]
for T in periods_plot:
    ...
    pisagua_df = functions_py.get_event_period_df(database,Fs,eqid,T,coeff_Rp,coeff_Rrup,p_value=p_opt_T,Rp_sheets=Rp_sheets,Rp_sheets_lock=Rp_lock_sheets,region = 'Global',Rref = Rref_gmm) 
    pisagua_df = pisagua_df.dropna(subset='log_obs_rock')
    obs_T = pisagua_df['log_obs_rock']
    mag = np.array(len(R_gmm)*[pisagua_df['Earthquake_Magnitude'].unique()[0]])
    R = pisagua_df['R']
    Rrup = pisagua_df['Rrup']
    Rp = pisagua_df['Rp']
    Rp_lock = pisagua_df['Rp_lock']
    ...
    with fig1.subplot(nrows = 1, ncols = 1, figsize=("4c","12c")):
        with fig1.set_panel(panel = [0,0]):
            fig1.basemap(region=[-73, -68,-22,-19], projection=projection_geo, frame=['WSne','xa1f.5','ya1f.5'])        
            pygmt.makecpt(cmap='/home/jtommy/Escritorio/graficos_GMT/paletas/grayscale02.cpt',series = [-30000,6000,10])
            fig1.grdimage(grid ='/home/jtommy/Escritorio/Respaldo/base_de_datos/Topobatimetrias/lock_nortegrande.grd',projection = projection_geo,
                        shading ='/home/jtommy/Escritorio/Respaldo/base_de_datos/Topobatimetrias/lock_nortegrande.int', cmap=True )
            ...
            fig1.colorbar(frame="xaf+lLocking degree",position="g-75.75/-22.7+w7c/0.5c+h",projection=projection_geo,region=[-76, -71,-22,-19])        
            fig1.text(text="a) LI15", position='TL', font="8p,Helvetica,black", projection=projection_geo,fill="white",pen = "0.25p,black,solid",offset='0.1/-0.1')                
            for i in modelos_cosismicos['event_tag']:
                ...
                #fig1.grdimage(grid =model_grid,projection = projection_geo, cmap=True,nan_transparent = True,region=[-73, -68,-22,-19]  ) 
                fig1.grdcontour(grid = model_grid,limit = [0,slip_th30],projection = projection_geo,annotation = "n",levels=slip_th30,pen='1p,'+colores[cont],region=[-73, -68,-22,-19])
                ...
                cont = cont+1      
            fig1.plot(x=pisagua_df['Station_Longitude_deg'], y=pisagua_df['Station_Latitude_deg'], fill='green',style='t0.2c',projection = projection_geo,pen="0.1p,black")      
    fig1.shift_origin(xshift="w+6c",yshift="-2.7c")
    with fig1.subplot(nrows = 3, ncols = 1, figsize=("4c","10c"),sharex=True):
        pygmt.config(FONT="6p,Helvetica,black")
        with fig1.set_panel(panel = [0,0]):
            fig1.basemap(region=[30, 1000,min(min(obs_T),min(pred_Rp),min(ln_mean_parker),min(ln_mean_montalva)) -0.5,max(max(obs_T),max(pred_Rp),max(ln_mean_parker),max(ln_mean_montalva))+1.5], projection=projection_log, frame=['Wsne','xa2fg3','ya2fg3'])
...
fig1.savefig('/home/jtommy/Escritorio/Respaldo/Paper1_v2/Predicciones/Pisagua/pred_pisagua.png',dpi=300,transparent=True)
fig1.savefig('/home/jtommy/Escritorio/Respaldo/Paper1_v2/Predicciones/Pisagua/pred_pisagua.pdf',dpi=300)