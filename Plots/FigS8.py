...
topo_grid = os.path.join(data_dir, 'melinka.nc')
topo_gradient = os.path.join(data_dir, 'melinka.int')
...
input_df = pd.DataFrame({
    "mag": len(R_gmm) * [melinka_database['Earthquake_Magnitude'].unique()[0]],
    "rrup": R_gmm,
    "vs30": len(R_gmm) * [760],
    "region": len(R_gmm) * ['SA'],
...
for T in periods_plot:
    print(f"Procesando Fig Melinka para T={T}")
    ...
    melinka_df = functions_py.get_event_period_df(
    ...
    R, Rrup = melinka_df['R'], melinka_df['Rrup']
    Rp, Rp_lock = melinka_df['Rp'], melinka_df['Rp_lock']
    ...
    with fig1.subplot(nrows=1, ncols=1, figsize=("4c", "12c")):
        with fig1.set_panel(panel=[0, 0]):
            ...
            for i in modelos_cosismicos['event_tag']:
                ...
                fig1.grdcontour(grid=model_grid, limit=[0, 0.4], projection=projection_geo, annotation="n", levels=0.4, pen='1p,blue')
                cont += 1
    ...
    with fig1.subplot(nrows=3, ncols=1, figsize=("4c", "10c"), sharex=True):
        ...
        y_min = min(min(obs_T), min(pred_Rp), min(ln_mean_parker), min(ln_mean_montalva)) - 1.2
        y_max = max(max(obs_T), max(pred_Rp), max(ln_mean_parker), max(ln_mean_montalva)) + 1
    ...
    fig1.savefig(os.path.join(output_dir, 'FigS8.pdf'), dpi=300)
