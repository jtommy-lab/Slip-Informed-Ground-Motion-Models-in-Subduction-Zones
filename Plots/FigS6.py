...
topo_grid = os.path.join(data_dir, 'lock_nortegrande.grd') 
topo_gradient = os.path.join(data_dir, 'lock_nortegrande.int') 
...
input_df = pd.DataFrame({
    "mag": len(R_gmm) * [pisagua_database['Earthquake_Magnitude'].unique()[0]],
    "rrup": R_gmm,
    "vs30": len(R_gmm) * [760],
    "region": len(R_gmm) * ['SA'],
    "saturation_region": len(R_gmm) * ['SA_S'],
    "backarc": len(R_gmm) * [False]
})
...
for T in periods_plot:
    print(f"Procesando Fig Pisagua para T={T}")
    ...
    R, Rrup = pisagua_df['R'], pisagua_df['Rrup']
    Rp, Rp_lock = pisagua_df['Rp'], pisagua_df['Rp_lock']
    ...
    with fig1.subplot(nrows=1, ncols=1, figsize=("4c", "12c")):
        with fig1.set_panel(panel=[0, 0]):
            ...
            if topo_gradient:
                ...
                fig1.grdimage(grid=topo_grid, projection=projection_geo, cmap=True)
            ...
            for i in modelos_cosismicos['event_tag']:
                ...
                fig1.grdcontour(grid=model_grid, limit=[0, 0.8], projection=projection_geo, annotation="n", levels=0.8, pen='1p,' + colores[cont], region=[-73, -68, -22, -19])
                cont += 1
    ...
    with fig1.subplot(nrows=3, ncols=1, figsize=("4c", "10c"), sharex=True):
        ...
        y_max = max(max(obs_T), max(pred_Rp), max(ln_mean_parker), max(ln_mean_montalva)) + 1.5
        
        
        with fig1.set_panel(panel=[0, 0]):
            fig1.basemap(region=[30, 1000, y_min, y_max], projection=projection_log, frame=['Wsne', 'xa2fg3', 'ya2fg3'])
            fig1.plot(x=Rp_lock, y=obs_T, style='c0.1c', fill='gray', pen='0.1p,black', intensity=0.3)
            
            fig1.plot(x=R_gmm, y=pred_Rp, pen='1p,red', label='This model')
    ...
    fig1.savefig(os.path.join(output_dir, 'FigS6.pdf'), dpi=300)
