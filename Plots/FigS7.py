...
topo_grid = os.path.join(data_dir, 'lock_centro.grd') 
topo_gradient = os.path.join(data_dir, 'lock_centro.int') 
...
input_df = pd.DataFrame({
    "mag": len(R_gmm) * [illapel_database['Earthquake_Magnitude'].unique()[0]],
    "rrup": R_gmm,
    "vs30": len(R_gmm) * [760],
    "region": len(R_gmm) * ['SA'],
...
for T in periods_plot:
    print(f"Procesando Fig Illapel para T={T}")
    ...
    illapel_df = functions_py.get_event_period_df(
    ...
    R, Rrup = illapel_df['R'], illapel_df['Rrup']
    Rp, Rp_lock = illapel_df['Rp'], illapel_df['Rp_lock']
    ...
    fig1.savefig(os.path.join(output_dir, 'FigS7.pdf'))
